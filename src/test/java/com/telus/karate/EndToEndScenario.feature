Feature: To create end to end scenario for all the 5 http methods
  ...
  Scenario: End to end scenario
    ...
    And print response
    * def validateMessage = read("ValidateDetails.json")
    And match response contains deep validateMessage
    And print "validateMessage", validateMessage
    Given path '/normal/webapi/all'
    And header Accept = 'application/xml'
    When method get # send the get request
    Then status 200 # response status code
    And print response
    * def validate = read("ValidateRequest.xml")
    And match response contains deep validate
    And print "validate", validate
    # Delete the job id
